...
export const DayTimeline = forwardRef<ScrollView, Props>(function DayTimeline(
  {
    events,
    dayOffset,
    nowPosition,
        ...
        <View style={{ height: timelineHeight }}>
          {Array.from({ length: END_HOUR - START_HOUR + 1 }, (_, index) => {
            ...
            return (
              <View
                key={hour}
                style={[styles.hourRow, { top: index * HOUR_HEIGHT }]}
              >
                <Text style={styles.hourLabel}>
                  {hour % 12 || 12} {hour < 12 ? AppText.am : AppText.pm}
                </Text>
          ...
          >
            ...
            {dayOffset === 0 &&
              ...
              nowPosition <= timelineHeight && (
                <View style={[styles.now, { top: nowPosition }]}>
...
function EventCard({
  ...
  const width =
    (availableWidth - EVENT_GAP * (event.columns - 1)) / event.columns;
  ...
  const finishDrag = useCallback(
    (translationY: number) => {
      ...
      const nextStart = Math.min(
        END_HOUR - duration,
        Math.max(START_HOUR, event.start + delta),
      );
  ...
  const dragGesture = useMemo(
    () =>
      Gesture.Pan()
        ...
        .onUpdate((gesture) => {
          dragY.value = gesture.translationY;
        })
    ...
    <GestureDetector gesture={dragGesture}>
      <Animated.View
        style={[
          styles.card,
          {
            ...
            left: event.column * (width + EVENT_GAP),
            width,
      ...
      >
        <Pressable onPress={() => onPress(event)} style={styles.cardContent}>
          <Text
            numberOfLines={1}
            style={[styles.eventTitle, event.completed && styles.completed]}
          >
            {event.completed ? AppText.completedPrefix : ""}
            {event.title}
          </Text>
          <Text style={styles.eventTime}>{formatDecimalTime(event.start)}</Text>
          {event.end - event.start > 1 && event.location && (
...
function EmptyDay() {
  return (
    <View style={styles.empty}>
      ...
      <Text style={styles.emptyTitle}>{AppText.clearDayTitle}</Text>
      <Text style={styles.emptyBody}>{AppText.clearDayMessage}</Text>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  scrollContent: { paddingTop: 8, paddingBottom: 80 },
  hourRow: {
    position: "absolute",
    left: 0,
    right: 0,
  ...
  eventTime: { color: "#696C73", fontSize: 10, marginTop: 2 },
  eventLocation: { color: "#777A82", fontSize: 10, marginTop: 5 },
  ...
  nowDot: { width: 8, height: 8, borderRadius: 4, backgroundColor: "#F04D4D" },
  nowLine: { height: 1.5, flex: 1, backgroundColor: "#F04D4D" },
  ...
  emptyTitle: { fontSize: 17, fontWeight: "700", color: "#2A2C32" },
  emptyBody: {
    fontSize: 13,
    color: "#85878E",
    textAlign: "center",
