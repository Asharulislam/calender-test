...
export function CalendarScreen() {
  ...
  const events = useMemo(
    () => layoutOverlappingEvents(eventsByDay[dayOffset] ?? []),
    [dayOffset, eventsByDay],
  );
  const now = new Date();
  const nowPosition =
    ...
    () =>
      PanResponder.create({
        ...
        onPanResponderRelease: (_, gesture) =>
          Math.abs(gesture.dx) > 65 && changeDay(gesture.dx < 0 ? 1 : -1),
      }),
    [changeDay],
  );
  ...
  const moveEvent = useCallback(
    (eventId: string, start: number, end: number) => {
      Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
      setEventsByDay((current) => ({
        ...current,
        [dayOffset]: (current[dayOffset] ?? []).map((event) =>
  ...
  const createEvent = (event: CalendarEvent) => {
    ...
    setEventsByDay((current) => ({
      ...current,
      [dayOffset]: [...(current[dayOffset] ?? []), event],
    }));
  ...
  return (
    <SafeAreaView style={styles.safeArea} edges={["top"]}>
      ...
      <DayTimeline
        ...
        onDragStateChange={setDraggingEvent}
      ...
      >
        <Ionicons name="add" size={30} color={AppColors.white} />
      </Pressable>
      <EventDetailsSheet
        event={selectedEvent}
