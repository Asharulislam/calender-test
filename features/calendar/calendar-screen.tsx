...
export function CalendarScreen() {
  ...
  const [eventsByDay, setEventsByDay] = useState<Record<number, CalendarEvent[]>>(() => ({ ...MOCK_EVENTS_BY_DAY }));
  const scrollRef = useRef<ScrollView>(null);
  const slideAnimation = useRef(new Animated.Value(0)).current;
  const date = dateAtDayOffset(dayOffset);
  const events = useMemo(() => layoutOverlappingEvents(eventsByDay[dayOffset] ?? []), [dayOffset, eventsByDay]);
  const now = new Date();
  ...
  const moveEvent = useCallback((eventId: string, start: number, end: number) => {
    Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
    setEventsByDay((current) => ({
      ...current,
      [dayOffset]: (current[dayOffset] ?? []).map((event) => event.id === eventId ? { ...event, start, end } : event),
    }));
  }, [dayOffset]);

  const createEvent = (event: CalendarEvent) => {
    Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
    setEventsByDay((current) => ({ ...current, [dayOffset]: [...(current[dayOffset] ?? []), event] }));
  ...
  return <SafeAreaView style={styles.safeArea} edges={['top']}>
    ...
    <DayTimeline ref={scrollRef} events={events} dayOffset={dayOffset} nowPosition={nowPosition} slideAnimation={slideAnimation} swipeResponder={swipeResponder} onEventPress={selectEvent} onEventDrop={moveEvent} onDragStateChange={setDraggingEvent} scrollEnabled={!draggingEvent} />
    <Pressable style={styles.addButton} onPress={() => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium); setCreatingEvent(true); }}><Ionicons name="add" size={30} color="white" /></Pressable>
    <EventDetailsSheet event={selectedEvent} onClose={() => setSelectedEvent(null)} />
    <CreateEventSheet visible={creatingEvent} onClose={() => setCreatingEvent(false)} onCreate={createEvent} />
  </SafeAreaView>;
}
